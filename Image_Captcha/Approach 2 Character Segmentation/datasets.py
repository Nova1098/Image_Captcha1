# -*- coding: utf-8 -*-
"""
Created on Fri Oct 13 03:30:26 2023
...
def getLabelDirectory(char):
    if char.islower():
        return char + "1"
    else:
        return encode_special_characters(char)
...
class CharactersDataset(torch.utils.data.Dataset):
    def __init__(self, data_root, validate = False):
        ...
        image_search_path = os.path.join(data_root, "**", "*.png")
        ...
        self.char_set = uppercase_letters + lowercase_letters + numbers + list(special_characters)
    ...
    def _load_image(self, index):
        ...
        img_label = decode_special_characters(img_label)
        img_label = img_label.rsplit("_", 1)[0] # convert from "A_1385.png" to "A_1385" to "A"
...
class CAPTCHADataset(torch.utils.data.Dataset):
    def __init__(self, data_root, img_format = "{}.png", size = -1):
        super(CAPTCHADataset, self).__init__()

        # root directory where the CAPTCHAs live
        self.data_root = data_root

        image_dir = os.path.join(self.data_root, "*.png")
    ...
    def _load_image(self, index):
        ...
        img_path = self.image_list[index]
        captcha_label = os.path.split(img_path)[1].split(".png")[0] # convert from 'data/38A7.png' to '38A7.png' to '38A7'
